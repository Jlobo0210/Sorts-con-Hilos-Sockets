...
public class Worker {
    ...
    // Variables adicionales para comunicación directa
    private static Socket otherWorkerSocket;
    private static ObjectOutputStream outToOtherWorker;
    private static ObjectInputStream inFromOtherWorker;

    public static void main(String[] args) throws IOException, InterruptedException, ClassNotFoundException {
        Work();
    }

    public static void Work() throws IOException, InterruptedException, ClassNotFoundException {
        try (Socket socket = new Socket(IP_ADDRESS, PORT)) {
            ObjectOutputStream out = new ObjectOutputStream(socket.getOutputStream());
            ObjectInputStream in = new ObjectInputStream(socket.getInputStream());
            System.out.println("Conectando al servidor en " + IP_ADDRESS + ":" + PORT);
               
            
            // Recibir información del otro worker
            String otherWorkerIP = (String) in.readObject();
            int otherWorkerPort = in.readInt();

            // Conectar al otro worker
            otherWorkerSocket = new Socket(otherWorkerIP, otherWorkerPort);
            outToOtherWorker = new ObjectOutputStream(otherWorkerSocket.getOutputStream());
            inFromOtherWorker = new ObjectInputStream(otherWorkerSocket.getInputStream());
            ...
            while (true) {
                ...
                try {
                    ...
                    if (elapsedTime >= timeLimit) {
                        // Enviar vector al siguiente worker
                        outToOtherWorker.writeObject(vector);
                        outToOtherWorker.flush();
                    } else {
                        // Enviar vector al servidor si está listo
